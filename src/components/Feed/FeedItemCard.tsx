...
export function FeedItemCard({ item, isLoggedIn }: FeedItemCardProps) {
  ...
  const status = representativeAnswer?.passStatus === 'PASS' ? 'passed' : representativeAnswer?.passStatus === 'FAIL' ? 'failed' : null;
  const passStatusLabel = status === 'passed' ? getLabel('PassStatus', 'PASS') : status === 'failed' ? getLabel('PassStatus', 'FAIL') : null;
  const tipPreview = representativeAnswer?.tip ?? null;
  const quotePreview = representativeAnswer?.content ? truncateText(representativeAnswer.content, 120) : undefined;
  ...
  return (
    <article className="rounded-xl bg-card p-4 space-y-3 border border-border/50">
      ...
      <div className="space-y-2">
        ...
        {tipPreview && (
          <p className="text-muted-foreground text-sm">{tipPreview}</p>
        )}
      ...
      <div className="flex flex-col gap-2 pt-2">
        ...
        {hasMembersOnly && (
          <p className="text-xs text-muted-foreground">
            로그인하면 답변 {answerCounts.membersOnlyAnswerCount}개를 더 볼 수 있어요.
          </p>
        )}
      </div>
    </article>
